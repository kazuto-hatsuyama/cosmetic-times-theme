...
| `sections/ranking-list.liquid` | 「売れ筋ランキング」ページ用の新規セクション（新規・2026-08-21、`feature/ranking-page`ブランチ）。データ系（`D:\Inetpub\shopify_data`）が投入した`shop.metafields.custom.sales_ranking`（list.product_reference、Shop単位・配列の並び順=ランキング順位）をループし、順位（`forloop.index`）・商品画像・商品名（リンク）・価格（`snippets/price.liquid`を`render`）を縦一列にSSR表示。1〜3位は`data-rank`属性＋CSSでゴールド/シルバー/ブロンズ配色に強調。件数はハードコードせず配列の実件数分ループ、0件/blankの場合は「ランキングを準備中です」と表示。タブ・アコーディオン不使用（`product-extra-info.liquid`等と同方針）。見出し文言は`heading`設定（デフォルト「売れ筋ランキング」）、`color_scheme`設定あり。`enabled_on.templates`は`["page"]` |
