...
| ファイル | 内容 |
|---|---|
| `assets/custom-luxury.css` | Luxury Design System v4.0（section19 は object-fit:cover）。**2026-09-04緊急修正**: ヘッダー下段ナビバー（`.header__navigation-bar-row`）の背景色指定に`!important`を追加（Horizon標準の`.header__row{background-color:transparent}`と同じ詳細度でロード順により後勝ちしていたため）。さらに子要素`.menu-list--mobile`（`color-scheme-1`により不透明な白背景を持つ）にも`background-color:transparent!important`を追加（子要素の白背景が親の茶色背景を完全に覆い隠していたため）。ユーザーが実ブラウザ（複数）でヘッダー下段ナビバーが完全に非表示（隙間のみ）になっていると報告し発覚。原因は前回デプロイ（`/collections/sale`リンク差し替え）自体ではなく、`sections/header-group.json`のnavigation_bar設定に起因する既存CSSの詳細度・カスケード順の潜在バグ（navigation_bar機能追加時からデプロイ未検証だった）。Playwright（ローカルnode_modules、`shopify theme dev`不使用）でストアパスワード突破後に実機DOM/computed styleを直接検証して特定・修正・再デプロイ後に解消を確認済み |
...
| `blocks/_header-menu.liquid` | `navigation_bar`ケースに新着(`/collections/new-arrivals`)・セール(`/collections/outlet`、専用コレクション未作成のため暫定)・ブランド(`/pages/brand-list`)・カテゴリ(`/pages/category-list`)の固定リンクを追加（新規・2026-09-01）。既存の`/pages/ranking`固定リンク（2026-08-21対応）と同じ方式・同じ理由（`main-menu`がAdmin側で空／Home・Catalog・Contactのみのため、`main-menu.links`をループしても何も表示されない。Admin GraphQL API`menuUpdate`は`write_online_store_navigation`スコープが無くACCESS_DENIED、実行不可のため理論側で固定リンク化）。5項目の並び順は新着→セール→ランキング→ブランド→カテゴリで現行SPサイトの`.head_menu`と一致させた。**2026-09-11: 「お悩みから探す」（`/pages/trouble-list`）の固定リンクを末尾に追加**（データ系からの依頼、`feature/mega-menu-trouble-link-ssr-audit`ブランチ。04_SE確認事項シートNo.1「メガメニューが閉じている状態でもブランド一覧・カテゴリ一覧・お悩み一覧がSSRで初期HTMLに含まれているか」の調査対応。調査の結果、Horizon標準のホバー展開メガメニュー（`.header-menu`/`.overflow-menu`、`snippets/mega-menu-list.liquid`）自体は`main-menu`にネストしたリンクが無いため常に空（「さらに表示する」ボタンのみ）で描画されており、実際にブランド一覧・カテゴリ一覧への導線として機能しているのはこの`navigation_bar`固定リンク（常時SSR・プレーンな`<a>`タグ、JS挿入なし）である一方、お悩み一覧（`/pages/trouble-list`、2026-08-25作成済み）だけがどこからもリンクされていないギャップが判明したため、既存4項目と同じ固定リンク方式で追加し解消した** |
| `snippets/header-drawer.liquid` | モバイルドロワー（ハンバーガーメニュー）末尾の固定リンクを、`/pages/ranking`のみ→新着/セール/ランキング/ブランド/カテゴリの5項目に拡張（新規・2026-09-01）。理由は`blocks/_header-menu.liquid`と同じ（`main-menu`が実質空でドロワーにも「ランキング」しか表示されていなかったため）。**2026-09-11: 同じ理由で「お悩みから探す」（`/pages/trouble-list`）を末尾に追加**（`blocks/_header-menu.liquid`の同日対応と同時実施、詳細は同ファイルの行を参照） |
| `sections/header.liquid` | 現行SPサイト`renew_header_s.cfm`の2段目（ログイン中会員向け挨拶文＋ランク表示＋ログアウト、未ログイン時は会員登録訴求）を再現する`header-member-bar-row`を新規追加（新規・2026-09-01、初めての本ファイル改変につき上記「テーマ標準ファイルへの変更」表にも追記要）。`customer`オブジェクトでログイン判定し、ランク名は`customer.metafields.custom.rank.value`（`snippets/cart-summary.liquid`と同じRankCD 00〜04のメタフィールド）を`sections/member-guide.liquid`に転記済みの名称（メンバー/フレンド/ハートフル/ラブリー/CT VIP）にマッピングして表示。ログアウトは`routes.account_logout_url`、未ログイン時の新規会員登録は`routes.account_register_url`を使用。**2026-09-04: 新規会員登録リンクを`routes.account_register_url`→`routes.storefront_login_url`に変更**（443行目）。理由: `account_register_url`は新Customer Accounts UI（Order index/Welcome画面）へ遷移し、ログイン後にテーマ側ページへ戻らないため、`layout/theme.liquid`の`{% render 'customer-sync' %}`（`customer-sync.js`→本番`link_customer.cfm`への会員同期）が発火しない不具合があった。`storefront_login_url`は`return_to`で現在のストアページを保持し、OTP認証完了後にテーマ側へ直接リダイレクトするため同期が確実に発火する（新規登録・既存ログイン共通の同一フロー）。もう一つのログイン導線（`snippets/header-actions.liquid`の`<shopify-account>`）はストアページ離脱がなく元から問題なし |
...
**ヘッダー・フッターSPデザイン統一の既知の制約（2026-09-01）**: 現行サイトの`renew_header_s.cfm`にある「問い合わせ」アイコン（`/guide/top`への専用リンク）・カート/新着/セールの「UP!」ピンクバッジ（表示日付入りバッジ）・検索窓は、Horizonの標準コンポーネント構成に対応する箇所がなく本タスクでは追加していない（検索アイコン自体はHorizon標準機能として別途存在）。同じく`new_footer_s.cfm`の「PCサイトへ」リンク（PC/SP別テンプレート運用前提の機能）はShopify側に存在しないため対象外。電話CTAボタンの丸型アイコン（電話マーク付き円形バッジ）もHorizonの`button`ブロックには対応するスロットがなく、テキストのみのボタンで代替した。**「セール」の遷移先（`/collections/outlet`）は暫定対応**：現行サイトの「セール」（EcDiscountFlg基準の値引き商品一覧、`/sale`）に対応する専用コレクションがShopify側に未作成のため、既存の`/collections/outlet`（アウトレット、`templates/index.json`のpromo_bannersで"OUTLET SALE"として既に使用）を代用している。データ系で専用の「セール」コレクションを作成後、`blocks/_header-menu.liquid`・`snippets/header-drawer.liquid`のリンク先を差し替える必要がある（`D:\Inetpub\HANDOFF.md`に申し送り記載）。**2026-09-04にデータ系が専用SmartCollection（handle=sale）を作成し`/collections/sale`へ一時的に差し替えたが、2026-09-07にユーザー指示で`/collections/outlet`へ再度差し戻した**（両ファイルのコメント・`D:\Inetpub\HANDOFF.md`参照。上記「暫定対応」の記述は結果的に現状と一致している）。**main-menu（Online Store > Navigation）自体はHome/Catalog/Contactのみで実質空のまま**（Admin API書き込み権限が無く本タスクでは編集不可）。新着/セール/ランキング/ブランド/カテゴリはナビバー・ドロワーいずれもテーマ側の固定リンクで表示しているため実害はないが、将来的にAdmin側でmain-menuに実項目を追加した場合、固定リンクと重複表示される可能性がある点に留意（`blocks/_header-menu.liquid`のコメント参照）。これらは1:1移植ができなかった箇所として記録する。

| `assets/custom-luxury.css` | 「21. カートページ」「22. 気になる！リスト（お気に入り一覧）ページ」セクションを新規追加（2026-09-01、`feature/dynamic-page-design-alignment`ブランチ、コミット`8242cb4`。**2026-09-02: mainへマージ（マージコミット`6ca7e2a`）・origin/mainへpush済み**。プレビューテーマID 139178737873でユーザー確認済み。マージによりGitHub Actions自動デプロイ（`workflow_dispatch`のみ）は発火せず、本番liveテーマへの反映は未実施）。データ系からの依頼で会員・購入系（動的ページ）のSP版デザイン移植の一環。**カート**（`sections/main-cart.liquid`・`blocks/_cart-summary.liquid`・`snippets/cart-summary.liquid`・`snippets/cart-products.liquid`はHorizon標準構造のまま変更せず）: 現行SP`new_cart_s.html.cfm`に合わせ、チェックアウトボタン（`.cart__checkout-button`）をピンク`#c83564`角丸ボタン（`.pop_cartbtn`相当）に、「ショッピングを続ける」ボタン（`.cart-items__empty-button`）をクリーム地・タウプ枠角丸（`.btn`相当）に、カート内商品の区切り線（`.cart-items--dividers .cart-items__table-row`）をモバイル幅のみドット線4px `#ccc`（`.cart_bdr`相当）に、合計金額（`.cart-totals__total-value`）をピンク文字に、カート合計ボックス（`.cart-summary`）をモバイル幅のみ白背景・枠線・角丸10px（`.reji_result`相当）に、ポイント/クーポン適用ボタンと削除ボタンホバー色をピンクアクセントに統一。**気になる！リスト**（`sections/favorite-list.liquid`の`snippets/resource-card.liquid`利用構造は変更せず）: 現行SP`list_item_favorite_s.html.cfm`に合わせ、モバイル幅で`.favorite-list__grid`をグリッドから縦1列のリストに変更、各`.favorite-list__item`内の`.resource-card`をflex-row化して画像35%・詳細右という横並びレイアウト（`.item .product`相当）に、カード間区切り線2px `#ccc`、0件時メッセージ背景を淡いピンク`#fbdce2`に変更 |

**カート・気になるリストSPデザイン統一の既知の制約（2026-09-01）**: `sections/favorite-list.liquid`は`snippets/resource-card.liquid`（商品タイトル・価格のみ描画）を利用しているため、現行SPにあるカテゴリタグ（ピンク文字）・星評価（クチコミ件数）・在庫状況アイコン（新着/セール/再入荷等のバッジ）・カート追加ボタン・「リストから削除」ボタンはこのタスクでは再現していない（resource-cardのデータ構造を拡張する、または専用スニペットを新規作成する場合は別タスクとして工数がかかるため見送り）。カートページの「以前カートに入れた商品」「カート内の商品と一緒に購入されてる商品」「最近チェックした商品」といったSP独自のレコメンドカルーセルも、Horizon標準の`product-list`セクション（`templates/cart.json`の`product_list_NNFgcy`、"You may also like"表示）とはデータソース・件数ロジックが異なるため今回は対象外（見出し文言・データ取得ロジックの作り込みが必要なため、CSSのみでの1:1再現は不可と判断）。ログイン/新規会員登録/マイページ/購入履歴の4ページはNew Customer Accounts仕様のため対象外（詳細は`D:\Inetpub\HANDOFF.md`参照）。**2026-09-02: この4ページについてはユーザーが「Shopify標準デザインのまま運用する」と最終決定**（`D:\Inetpub\HANDOFF.md`記録済み）。テーマ側でのデザイン移植対応は今後不要（既知の制約として恒久的にクローズ）。

## ⚠️ 今後の標準原則: 「SP版参照」の解釈（2026-09-02〜）

ユーザーから、デスクトップヘッダーの実機スクリーンショット（`set.png`）を見て「会員名が見えない・フォントが小さい・デザインがダサい」と強い指摘があった。原因は、これまで「SP版（現行サイト`renew_header_s.cfm`等）を参考にする」という指示を、配色・フォントサイズ・余白まで含めてSP版のCFMコードに忠実に寄せる、という意味で実装してきたこと。**今後は以下の解釈に統一する**:

- **表示内容（コンテンツ）はSP版に合わせる**: 何の情報をどこに出すか（会員名＋ランク表示、新着/セール/ランキング/ブランド/カテゴリのナビ項目など）は維持する
- **ビジュアルデザインはSP版のCFMコードを模倣しない**: 配色・フォントサイズ・余白・背景色・階層表現は、Horizonテーマが既に他のページ・セクションで使っているデザイン言語（角丸ピル型ボタン、ブランドアクセント＝ピンク`#c83564`、カード型の余白感）に合わせて作り直す

| `assets/custom-luxury.css` | 「23. ヘッダー・ナビバー・フッター ビジュアル刷新」セクションを新規追加（2026-09-02、`feature/header-footer-visual-refresh`ブランチ）。上記の指摘・方針転換を受けた対応。**ヘッダー会員バー**（`.header-member-bar-row`）: 白背景・0.8rem・下線リンクのみでロゴ行と一体化して見えていたのを、クリーム地（`#faf8f4`、フッター`scheme-8`と同トーン）＋上下罫線で明確に区切り、ランクは塗りピルバッジ（ピンク地・白文字）、ログアウト/新規会員登録はピンク枠のピルボタン化。**ヘッダー下段ナビバー**（`.header__navigation-bar-row`）: 背景色を単調だったオリーブ`#635d4d`からフッター最下部（`scheme-9`）と同系の深いウォームブラウン`#4a4537`に変更しヘッダー・フッターの統一感を演出、文字サイズを0.95rem・太字に拡大、項目間に縦区切り線、ホバー時にピンクの下線がせり上がるアニメーションを追加して「押せる」ことを明示。**上段ヘッダー行**: 構造・アイコンはHorizon標準のまま変更せず、ホバー時の色のみブランドアクセント（ピンク）に統一（`#header-component .header-actions__action:hover`）。**フッター**: 電話CTAボタンをタウプブラウン地からピンク枠のアウトラインボタン（ホバーで塗りつぶし、カートページCTAと統一）に変更、SNSアイコンを26px・間隔20pxに拡大。**既存バグ修正**: 「15. Footer」セクションの`[class*="footer"] a`（文字色を薄いグレーに固定する広範なセレクタ）が、著作権バー（`footer-utilities`、`scheme-9`・背景ダークブラウン`#524e43`）内のリンクにも祖先セレクタとして効いてしまい、暗い背景に暗いグレー文字がほぼ判読できない状態になっていたため、`.color-scheme-9 a`に白文字・ホバー時ピンクを明示的に上書きする対応を追加。同セクションのリンクホバー色もゴールド`#C9A84C`からブランドアクセントのピンク`#c83564`に統一（ブランドカラーはゴールドではなくピンクという既存方針との不整合を修正）。**settings_data.json非連動の注意**: `scheme-7`のCSS変数（`--color-background`等）はJS/Liquid側からは参照されずCSS上書きのみで完結するため、`config/settings_data.json`側の`scheme-7`定義（gitignore対象、背景`#635d4d`のまま）を更新する必要はない（従来のsection 20と同じ設計方針を踏襲）。**2026-09-02: 実機確認（プレビューテーマID 139186569425）で判明した2件の追加修正（上段ヘッダーの重複「ランキング」リンク削除`blocks/_header-menu.liquid`、会員バー背景の`.header__row`透過バグ修正）を経てユーザー承認、mainへマージ（マージコミット`9179769`）・origin/mainへpush済み**。マージによりGitHub Actions自動デプロイ（`workflow_dispatch`のみ）は発火せず（`gh run list`で確認済み、直近の自動発火は`workflow_dispatch`化前の`59912fe`が最後）、本番liveテーマ（138815832273）への反映は未実施 |

**本タスクで実機スクリーンショットによるBefore/After確認ができなかった制約（2026-09-02）**: 作業環境にChrome DevTools MCP・Playwright MCPのいずれも実際には接続されておらず（`CLAUDE.md`記載の設定はあるが本セッションではツールとして呼び出し不可だった）、ブラウザでの実機確認・スクリーンショット取得を行っていない。今回の修正はコードレビュー（既存CSSの詳細度・カスケードの読み解き、`shopify theme check`によるlint）ベースで、上記「既存バグ修正」の項目（著作権バーの文字色）は特に、DOM構造の祖先関係から論理的に導いた診断であり実機未検証。**マージ前に必ずプレビューテーマで実機確認すること**（特に: ヘッダー会員バー・ナビバーの見た目、フッター著作権バーの文字が実際に読めるようになっているか、電話CTAボタンの見た目）。

**2026-09-02（ユーザーの実機確認後の追加修正、2件）**:

1. `blocks/_header-menu.liquid`: 上段の白いヘッダー行（ロゴのすぐ右）に、下段ナビバーと重複する「ランキング」単独リンクが地味な見た目で表示されていた不具合を修正。原因はAdmin管理の`main-menu`がHome/Catalog/Contactのみで実質空のため、2026-08-21対応の固定リンク「ランキング」がデスクトップの上段メニュー（`else`分岐）で唯一表示される項目になっていたこと。下段ナビバーが2026-09-01に常時表示化されたことで完全に重複していたため、上段側の固定`<li>`を削除（下段ナビバー・モバイルドロワー側の同等リンクは維持しているため導線自体は失われない）。Admin側main-menuの編集自体は引き続きAPI権限不足（`write_online_store_navigation`スコープなし）で本タスクでは対応不可。
