...
| `assets/favorite-button.js` | 上記ブロックのクリック挙動を担当する新規Web Component（`favorite-button-component`、新規・2026-08-27）。`assets/customer-sync.js`と同じ共有token（`bee15c758842afe80a460a8d1d899e88323163105c99946f`）・`ngrok-skip-browser-warning`ヘッダー・エンドポイントホスト（`https://arnulfo-fordable-pipingly.ngrok-free.dev/Manage/shopify/`）を使い回す（データ系了承済みの既存方式を踏襲、詳細・恒久化の必要性は下記「customer-syncの外部エンドポイント」の未対応事項と同じ注意点が当てはまる）。ログイン済みの場合のみこのコンポーネントがDOMに存在する（未ログイン時は`blocks/favorite-button.liquid`側で通常リンクとして描画されAPIは呼ばれない設計のため、このJS内では未ログイン分岐を持たない） |
...
| お気に入り機能の外部エンドポイント（`toggle_favorite.cfm`） | `assets/favorite-button.js`も`customer-sync.js`と同じngrok無料枠エンドポイント・ベタ書きtokenを使い回している。上記「customer-syncの外部エンドポイント」と同じ理由・同じ懸念（URL変更/停止リスク、本番公開前の恒久化推奨）がそのまま当てはまる |
